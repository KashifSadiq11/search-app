#!/usr/bin/env python3
"""
Import BuyPass Keycloak users into the RecEngine `users` table.

Hybrid logic (DB + existing API):

- First, work ONLY by email:

  1) Check in the DB (direct SQL) if a row with this email exists
     (case-insensitive: lower(email) = lower(import_email)).

     - If email exists:
         -> UPDATE that row (direct SQL):
                username = email
                email    = email
         -> Do NOT create any new user.

  2) If email does NOT exist:
         -> Call FastAPI POST /users/ to create a new user with:
                id       = userId (from JSON)
                email    = email
                username = email

         -> If POST /users/ returns a 400 with a PK conflict on id:
                - Treat it as "user with this id already exists"
                - Then UPDATE that row directly in DB by id:
                      username = email
                      email    = email

So:
- Email decides whether it's "existing vs new".
- We use DB directly for email lookups + updates.
- We use the existing /users/ API for creating new users.
"""
...
API_URL = "http://localhost:8000"   # uses existing /users/ endpoint
...
def convert_keycloak_user(user: Dict) -> Dict:
    """
    Convert a Keycloak user to payload for DB/API.
...
def import_users(users_json: str) -> Tuple[List[Dict], List[Dict], List[Dict]]:
    ...
    created: List[Dict] = []           # new users (via API /users/)
    updated_existing: List[Dict] = []  # existing rows updated (via direct DB)
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            for i, raw_user in enumerate(users):
                ...
                # ---------------------------------------------------------
                # 2) EMAIL-FIRST (DB ONLY): does a user with this email exist?
                # ---------------------------------------------------------
                ...
                if existing:
                    ...
                    # Update existing row by id (DB DIRECT)
                    try:
                        cur.execute(
                            """
                            UPDATE users
                    ...
                    # NO new user created when email exists
                    continue

                # ---------------------------------------------------------
                # 3) EMAIL NOT FOUND → create NEW user via API POST /users/
                # ---------------------------------------------------------
                ...
                try:
                    resp = requests.post(f"{API_URL}/users/", json=api_payload)
                except requests.RequestException as e:
                    err_str = f"HTTP error calling POST /users/ for id={user_id}: {e}"
                    print(f"✗ [{n}/{len(users)}] {err_str}")
                    logger.error(err_str)
                    failed.append({"raw": raw_user, "error": err_str})
                    continue
                ...
                if resp.status_code in (200, 201):
                    ...
                    created.append(body)
                    print(
                        f"✓ [{n}/{len(users)}] Created NEW user {username} ({email}) via API"
                    )
                    logger.info(
                        "Created new user via API [%s/%s]: id=%s, email=%s",
                        n,
                        len(users),
                        user_id,
                        email,
                ...
                # Handle potential PK conflict from API (id already exists)
                if resp.status_code == 400 and "duplicate key value violates unique constraint \"users_pkey\"" in resp.text:
                    print(
                        f"↻ [{n}/{len(users)}] API insert hit PK conflict for id={user_id}. "
                        f"Updating username/email by id via DB."
                    )
                    logger.warning(
                        "PK conflict from API (users_pkey) for id=%s, updating instead",
                        user_id,
                    )
                    try:
                        cur.execute(
                        ...
                        conn.commit()
                        updated_existing.append(
                            {"id": user_id, "email": email, "reason": "by_id_conflict_from_api"}
                        )
                    except Exception as e2:
                        conn.rollback()
                        err_str = (
                            f"Failed to update existing user by id after API PK conflict "
                ...
                # Other API errors
                err_str = (
                    f"POST /users/ failed for id={user_id}, email={email} | "
