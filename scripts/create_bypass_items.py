...
def convert_buypass_to_recengine(buypass_product: Dict) -> Dict:
    ...
    import re
    description = buypass_product.get("productDescription", "")
    main_description = buypass_product.get("productMainDescription", "")
    
    # Remove HTML tags
    description = re.sub('<.*?>', '', description).strip()
    main_description = re.sub('<.*?>', '', main_description).strip()
    
    # Combine descriptions intelligently
    if main_description and main_description != description:
        full_description = f"{main_description}. {description}"
    else:
    ...
    product_status = status_mapping.get(
        buypass_product.get("productStatus", "active"), 
        "active"
    )
    ...
    metadata = {
        ...
        "sold_count": buypass_product.get("soldCount", 0) + buypass_product.get("totalSold", 0)
    }
    
    # Create enhanced description with metadata hint
    enhanced_description = f"{full_description}\n[Metadata: {json.dumps(metadata, separators=(',', ':'))}]"
...
def import_products(products_json: str):
    ...
    if successful_imports:
        with open("imported_items.json", "w") as f:
...
if __name__ == "__main__":
    # Read from JSON file
    with open('/mnt/f/sistemsai/recomendations/bypass_data.json', 'r') as f:
    ...
    if successful:
        ...
        if test_search.status_code == 200:
            ...
            print(f"Search for 'phone' found {results['total']} items")
